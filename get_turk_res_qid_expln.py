...
def valid_row(row):
    ...
    for i in range(args.num):
        ...
        for opt in ANSWER_OPTIONS:
            total_sum += row.get(get_key_answer(opt,i),0)
    if(total_sum != args.num):
        return 'You did not mark any option in some questions'
    if(quality_ctrl_id is not None):
        if(not (row.get(get_key_answer('both',quality_ctrl_id),False) or row.get(get_key_answer('none',quality_ctrl_id),False))):
    ...
    for df in dflist:
        ...
        for index, row in df.iterrows():
            for i in range(args.num):
                ...
                explanation = row['Answer.explanation'+str(i)]
                if(exp_A == exp_B):
                    continue
                ...
                if(fact not in results):
                    results[fact] = {'our_exp': exp_A,'other_exp':exp_B, 'answers' : [],'row_idx':[], 'fact_no':[],'qid': qid_from_book,'explanation':[],'ours_is': our_expln}
                ...
                for opt in ANSWER_OPTIONS:
                    if(row.get(get_key_answer(opt,i),False)):
