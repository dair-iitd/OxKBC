...
def write_english_exps(mapped_data, exps, output_path, num_per_hit, explainer,y_labels):
    ...
    print('Generated csv_data {} and {}'.format(len(csv_data_exps),len(csv_data_no_exps)))
    ...
    df.to_csv(os.path.join(output_path,last_out_part+"_exps_hits.csv"), index=False, sep=',')
    ...
    df.to_csv(os.path.join(output_path,last_out_part+"_no_exps_hits.csv"), index=False, sep=',')
...
if __name__ == "__main__":
    ...
    if(args.template_pred is not None):
        exps = english_exp_template(mapped_data, template_predictions, template_objs,explainer)
