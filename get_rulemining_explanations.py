...
def write_english_exps(mapped_data, rule_exps_1, rule_exps_2, rule_exps_3, output_path, num_per_hit, explainer,qids):
    raw_data = queue.Queue(0)
    both_no = 0
    both_same = 0
    not_in_qidlist = 0
    qlty_ctrl = queue.Queue(0)
    total_exp_data = list(zip(mapped_data, rule_exps_1,rule_exps_2,rule_exps_3, list(range(len(mapped_data)))))
    random.shuffle(total_exp_data)
    for fact, r_exp1, r_exp2, r_exp3, q_id in total_exp_data:
        if q_id not in qids:
            not_in_qidlist += 1
            continue
        #
        # if(r_exp[0] == explainer.NO_EXPLANATION and r_exp[1] == explainer.NO_EXPLANATION and r_exp[2] == explainer.NO_EXPLANATION):
        #     both_no += 1
        #     continue
        htmled_fact = explainer.html_question(fact)
        row = [htmled_fact, r_exp1, r_exp2, r_exp3, q_id]
        # if(t_exp == r_exp):
        #     both_same += 1
        #     qlty_ctrl.put(row)
        #     continue
    ...
    df_html = pd.DataFrame(html_data, columns=['fact', 'rule_1', 'rule_2', 'rule_3','qid'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-rp1', '--rule_pred_1',
                        help='List of rules predicted for data(type 1)', default=None)
    parser.add_argument('-rp2', '--rule_pred_2',
                        help='List of rules predicted for data(type 2)', default=None)
    parser.add_argument('-rp3', '--rule_pred_3',
                        help='List of rules predicted for data(type 3)', default=None)
    ...
    rule_predictions_1 = utils.read_pkl(args.rule_pred_1)
    logging.info("Loaded rule predictions  from %s" % (args.rule_pred_1))
    rule_predictions_2 = utils.read_pkl(args.rule_pred_2)
    logging.info("Loaded rule predictions  from %s" % (args.rule_pred_2))
    rule_predictions_3 = utils.read_pkl(args.rule_pred_3)
    logging.info("Loaded rule predictions  from %s" % (args.rule_pred_3))

    if(len(rule_predictions_1) != len(mapped_data)):
        logging.error("Unequal length of rule predictions and data of type 1")
        exit(-1)
    if(len(rule_predictions_2) != len(mapped_data)):
        logging.error("Unequal length of rule predictions and data of type 2")
        exit(-1)
    if(len(rule_predictions_3) != len(mapped_data)):
        logging.error("Unequal length of rule predictions and data of type 3")
        exit(-1)
    ...
    template_objs = template_builder.template_obj_builder(
        data_root, args.model_weights, args.template_load_dir, None, "distmult", [1], True)
    ...
    rule_exps_1 = english_exp_rules(mapped_data, rule_predictions_1, explainer)
    rule_exps_2 = english_exp_rules(mapped_data, rule_predictions_2, explainer)
    rule_exps_3 = english_exp_rules(mapped_data, rule_predictions_3, explainer)
    ...
    print(qids)
    write_english_exps(mapped_data, rule_exps_1, rule_exps_2, rule_exps_3,
